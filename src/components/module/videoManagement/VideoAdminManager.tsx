  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        if (statusFilter === 'ALL') {
          const response = await axiosInstance.get(`/video/admin`);
          ...
          const response = await axiosInstance.get(`/video/admin?status=${status}`);
          const raw = response.data?.data?.items || response.data?.data || (Array.isArray(response.data) ? response.data : []);
          allVideos = raw;
        }
...
const handleAcceptOrRejectVideo = async (videoId: string, action: 'accept' | 'reject') => {
  try {
    ...
    const response = await axiosInstance.patch(`/video/${videoId}/moderate`, {
      reason: reason,
      action: action,
    }, {
